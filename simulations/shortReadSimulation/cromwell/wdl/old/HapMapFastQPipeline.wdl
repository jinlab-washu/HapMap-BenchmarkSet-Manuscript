...
workflow HapMapFastqSim {
    ...
    Array[File] ReferenceLists=read_lines(ReferenceFOFN)
    ...
    scatter (i in range(Runs)) {
        ...
        ##scatter by aligment
        scatter (references in ReferenceLists) {

            Array[File] allReferences=read_lines(references)

            call polyg.RemovePolyG as RemovePolyG {
                input:
                    Fastq=[combineAllFoward.combinedFile, combineAllReverse.combinedFile],
            ...
            call bwa.BwaAlignment as BwaAlignment {
                input:
                    sample=allReferences[8],
                    Fastq=RemovePolyG.out,
                    Reference=allReferences[0],
                    RefFAI=allReferences[1],
                    RefAMB=allReferences[3],
                    RefANN=allReferences[4],
                    RefBWT=allReferences[5],
                    RefPAC=allReferences[6],
                    RefSA=allReferences[7],
                    BwaDocker=BwaDocker
            }
        }
    }
    ...
    scatter (references in ReferenceLists) {
        Array[File] allReferences=read_lines(references)
        String ReferenceNameList=allReferences[8]
    }

    Array[Array[File]] transposed = transpose(BwaAlignment.AlignedFile)

    scatter (i in range(length(ReferenceLists))) {
        String FinalBamName="HapMap_${TotalReadDepth}x_${ReferenceNameList[i]}_aligned.bam"
    ...
    output {
        Array[File] SimulatedBam = SortAndIndex.OutBam
        Array[File] SimulatedIndex = SortAndIndex.Index
    }
}
