...
precision_plot <- ggplot(precision_combined_data, 
                         aes(x=file, y=Precision, color=Caller, linetype=Genome, shape=Genome, group=interaction(Caller, Genome))) +
  geom_point(size=5) +
...
sensitivity_plot <- ggplot(sensitivity_combined_data, 
                           aes(x = file, y = Sensitivity, color = Caller, 
                               group = interaction(Caller, Genome), shape=Genome, linetype=Genome)) +
  geom_point(size=5) +
  geom_line(linewidth=1) +
  scale_color_manual(values = color_pal) +
...
precision_plot <- ggplot(precision_combined_data,
  ...
  scale_shape_manual(values = c(CHM13 = 16, GRCh38 = 17)) +  # 16 = filled circle, 17 = triangle
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  facet_grid(. ~ VarType) +
...
sensitivity_plot <- ggplot(sensitivity_combined_data,
  ...
  ylim(0, 1) +
  scale_x_discrete(drop = FALSE) +
  scale_shape_manual(values = c(CHM13 = 16, GRCh38 = 17)) +  # match above
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  facet_grid(. ~ VarType) +
  theme(panel.border = element_rect(color = "black", fill = NA, size = 1),
