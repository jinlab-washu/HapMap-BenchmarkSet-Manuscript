...
# Run RTG vcfeval for SNV call sets against truth sets.
#
# Usage:
#   bash 05_run_snv_compare.sh
#
...
OUT_DIR="${OUT_DIR:-/storage2/fs1/epigenome/Active/shared_smaht/TEST_RUN_FOLDER/TrueMutSet/nahyun_tmp/caller_evaluation/snv}"
...
for variant in "${!snv_files[@]}"; do
    ...
    if [[ "${variant}" == *_CHM13 ]]; then
        ...
        truth_af_prefix="${TRUTH_DIR}/truth_set_${VERSION}_snvs_CHM13"
    else
        sdf="${SDF_HG38}"
        truth="${TRUTH_HG38}"
        reliable="${RELIABLE_HG38}"
        truth_af_prefix="${TRUTH_DIR}/truth_set_${VERSION}_snvs_GRCh38"
    fi
    ...
    for id in {1..9}; do
        rtg vcfeval -T 10 \
            ...
            --evaluation-regions "${reliable}" \
            --squash-ploidy \
            --output "SNV_${variant}_af_${id}_sensitivity"
    done
    ...
    for id in {1..9}; do
        rtg vcfeval -T 10 \
            --template "${sdf}" \
            --baseline "${truth}" \
            --calls "${QUERY_DIR}/${variant}_snvs_${id}.vcf.gz" \
            --sample "HapMap_Mixture,${sample_name}" \
            --evaluation-regions "${reliable}" \
            --squash-ploidy \
            --output "SNV_${variant}_af_${id}_precision"
    done
done

echo "All SNV evaluations completed."
