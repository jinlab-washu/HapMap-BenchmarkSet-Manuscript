...
# Run RTG vcfeval for INDEL call sets across k-mer mappability regions.
#
# Usage:
#   bash 06_run_indel_compare_kmer.sh
#
...
OUT_DIR="${OUT_DIR:-/storage2/fs1/epigenome/Active/shared_smaht/TEST_RUN_FOLDER/TrueMutSet/nahyun_tmp/caller_evaluation/indels/kmer}"
...
for variant in "${!indel_files[@]}"; do
    ...
    for id in k24.umap k36.umap k50.umap k100.umap all; do
        rtg vcfeval -T 10 \
            ...
            --squash-ploidy \
            --all-records \
            --output "INDEL_${variant}_kmer_${id}"
    done
    ...
    if [[ "${variant}" == *_CHM13 ]]; then
        rtg vcfeval -T 10 \
            ...
            --squash-ploidy \
            --all-records \
            --output "INDEL_${variant}_kmer_all_w_chm13only"
        ...
        rtg vcfeval -T 10 \
            ...
            --squash-ploidy \
            --all-records \
            --output "INDEL_${variant}_kmer_all_wo_chm13only"
    fi
done

echo "All INDEL k-mer evaluations completed."