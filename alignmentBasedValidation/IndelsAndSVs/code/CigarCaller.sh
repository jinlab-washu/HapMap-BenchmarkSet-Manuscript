...
then
    ...
    echo "Correct usage: bash BetterMpileup.sh {Indel/SV} {Bam File} {Reference File} {if LRS or Not (True/False)} {run name} {benchmarkset} {Out Directory}"
    exit 53
fi
...
for chrom in chr1 chr2 chr3 chr4 chr5 chr6 chr7 chr8 chr9 chr10 chr11 chr12 chr13 chr14 chr15 chr16 chr17 chr18 chr19 chr20 chr21 chr22 chrX chrY; do
    bash /storage1/fs1/jin810/Active/testing/Ruttenberg/Code/FinalSMAHTCode/CigarCaller/CigarChecker.sh $benchmark $bam $ref $LRS $indel "${outdir}" "${outdir}/outputVCFs/${name}_${chrom}.vcf" $chrom
done
